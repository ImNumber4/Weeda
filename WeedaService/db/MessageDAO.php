...
class MessageDAO extends BaseDAO
{
	...
	public function query($user_id) {
		$query = 'SELECT message.id as id, sender.username as sender_username, receiver.username as receiver_username, sender_id, receiver_id, message, message.time as time, message.deleted as deleted, type, related_weed_id, is_read FROM message LEFT JOIN user as sender on message.sender_id = sender.id LEFT JOIN user as receiver on message.receiver_id = receiver.id WHERE (receiver_id = ' . $user_id . ' or sender_id = ' . $user_id . ')';
