...
@property (nonatomic, retain) UIView *backgroud;
...
- (id)init
{
    ...
    _backgroud = [[UIView alloc]initWithFrame:CGRectMake(0, 0, CGRectGetWidth([UIScreen mainScreen].bounds), CGRectGetHeight([UIScreen mainScreen].bounds))];
    _backgroud.backgroundColor = [UIColor blackColor];
    _backgroud.alpha = 0.0;
    
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(cancelActoinSheet)];
    [_backgroud addGestureRecognizer:tapGesture];
            ...
            otherButtonTitles:(NSString *)otherButtonTitles, ...
{
    ...
    if (cancelButtonTitle) {
        WLActionSheetButton *cancelButton = [[WLActionSheetButton alloc]initWithAllCornersRounded];
    ...
    switch (titles.count) {
        case 0:
            break;
        case 1: {
            WLActionSheetButton *otherButton = nil;
            if (title) {
                otherButton = [[WLActionSheetButton alloc]initWithBottomCornersRounded];
            } else {
                otherButton = [[WLActionSheetButton alloc]initWithAllCornersRounded];
            }
        ...
        case 2: {
            WLActionSheetButton *lastOtherButton = [[WLActionSheetButton alloc]initWithBottomCornersRounded];
            [lastOtherButton setTitle:[titles lastObject] forState:UIControlStateNormal];
            
            WLActionSheetButton *secoundButton;
            if (title) {
                secoundButton = [[WLActionSheetButton alloc] init];
            } else {
                secoundButton = [[WLActionSheetButton alloc] initWithTopCornersRounded];
            }
        ...
        default: {
            ...
            WLActionSheetButton *firstOtherButton;
            if (title) {
                firstOtherButton = [[WLActionSheetButton alloc]init];
            } else {
                firstOtherButton = [[WLActionSheetButton alloc]initWithTopCornersRounded];
            }
            ...
            for (int i = 1; i < (titles.count - 2); i++) {
                WLActionSheetButton *middleButton = [[WLActionSheetButton alloc]init];
...
- (void)showInView:(UIView *)view
{
    ...
    [view insertSubview:_backgroud belowSubview:self];
...
- (void)setupActionSheetFrame
{
    ...
    if (_hasCancelButtion) {
        WLActionSheetButton *button = [_buttons lastObject];
    ...
    for (int i = 0; i < buttonCountWithoutCancel; i++) {
        WLActionSheetButton *button = [_buttons objectAtIndex:i];
        [self addSubview:button];
...
{
    for (WLActionSheetButton *button in _buttons) {
        if ([button isKindOfClass:[WLActionSheetButton class]]) {
...
- (void)buttonPress:(WLActionSheetButton *)button
{
...
- (void)removeView
{
    [UIView animateWithDuration:0.3f delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        [_backgroud removeFromSuperview];
        [self removeFromSuperview];
    }];
}

- (void)buttonHightlight:(WLActionSheetButton *)button
{
    button.backgroundColor = [ColorDefinition grayColor];
}

@end

@implementation WLActionSheetButton

@synthesize index;

- (id)init
{
    self = [self initWithFrame:CGRectMake(0, 0, CGRectGetWidth([UIScreen mainScreen].bounds) - GAP_BETWEEN_CANCEL, DEFAULT_BUTTON_HEIGHT)];
    self.backgroundColor = [UIColor whiteColor];
    self.titleLabel.font = [UIFont systemFontOfSize:21];
    [self setTitleColor:[ColorDefinition greenColor] forState:UIControlStateNormal];
